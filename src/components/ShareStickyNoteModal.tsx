...
import {
  getStickyNoteShares,
  revokeStickyNoteShare,
  shareStickyNote,
  updateStickyNoteShareRole,
} from "../services/stickyNoteSharing";
import { findUserByEmailOrUsername } from "../services/users";
import type { ShareRole, StickyNote } from "../types";

interface ShareStickyNoteModalProps {
  isOpen: boolean;
  onClose: () => void;
  note: StickyNote | null;
}

export default function ShareStickyNoteModal({
  isOpen,
  onClose,
  note,
}: ShareStickyNoteModalProps) {
  const { state } = useApp();
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setLoading(true);
      try {
        const data = await getStickyNoteShares(note.id);
        setShares(data);
      } catch (e) {
        console.error(e);
      } finally {
  ...
  const handleAdd = async () => {
    ...
    setLoading(true);
    const res = await shareStickyNote({
    ...
    queryClient.invalidateQueries({ queryKey: ["sharedStickyNoteIds"] });
  };

  const handleChangeRole = async (shareId: string, newRole: ShareRole) => {
    try {
      await updateStickyNoteShareRole(shareId, newRole);
  ...
  const handleRevoke = async (shareId: string) => {
    try {
      await revokeStickyNoteShare(shareId);
